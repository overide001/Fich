...
class Eco{
  ...
  reset(playerLineage){
    this.fish.length=0;this.food.length=0;
    ...
    for(let i=0;i<CFG.FOOD_TGT;i++)this.spawnFood();
    ...
    for(const linKey of LINEAGE_KEYS){
      if(linKey===playerLineage){
        for(let i=0;i<3;i++)this.spawnFish(linKey,rand(4,9));
        for(let i=0;i<2;i++)this.spawnFish(linKey,rand(13,20));
      } else {
        for(let i=0;i<4;i++)this.spawnFish(linKey,rand(4,9));
        for(let i=0;i<2;i++)this.spawnFish(linKey,rand(13,20));
  ...
  spawnFood(){
    if(this.food.length>=CFG.FOOD_TGT)return;
    this.food.push({
      pos:{x:rand(30,CFG.W-30),y:rand(30,CFG.H-30)},
      r:rand(0.9,1.8),
      phase:Math.random()*Math.PI*2,
      drift:{x:rand(-4,4),y:rand(-4,4)},
    });
  }
  spawnFish(lineageKey,size){
  ...
  update(dt){
    ...
    while(this.foodAccum>=1){this.foodAccum-=1;this.spawnFood();}
  ...
  balance(){
    ...
    const pc=prey+mid;
    if(pc<16)for(let i=0;i<rInt(2,4);i++)this.spawnFish(LINEAGE_KEYS[rInt(0,4)],rand(4,9));
